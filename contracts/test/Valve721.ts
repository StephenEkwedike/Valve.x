  ...
  describe("Valve721", function () {
    ...
    describe("check for eth", () => {
      ...
      it("create", async function () {
        await valve.connect(tester1).createTransfer(token.address, tester2.address, 0, tokenData, { value: fee });
      ...
      it("create more", async function () {
        await valve.connect(tester1).createTransfer(token.address, tester3.address, 1, tokenData, { value: fee });
        await valve.connect(tester1).createTransfer(token.address, tester4.address, 2, tokenData, { value: fee });
        await valve.connect(tester1).createTransfer(token.address, tester5.address, 3, tokenData, { value: fee });
